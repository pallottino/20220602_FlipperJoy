 ...
 */
extern const uint8_t hid_rpt01 [HID_RPT01_SIZE];
...
//Make sure "#define USB_CTRLDEVICE_HID_IDLE_RATE_CALLBACK(reportID, newIdleRate)    USBHIDCBSetIdleRateHandler(reportID, newIdleRate)"
...
#ifndef USB_CTRLDEVICE_HID_IDLE_RATE_CALLBACK
    #define USB_CTRLDEVICE_HID_IDLE_RATE_CALLBACK(reportId, idleRate)
#else
    extern void USB_CTRLDEVICE_HID_IDLE_RATE_CALLBACK(uint8_t reportId, uint8_t idleRate);
#endif
...
void USBCheckHIDRequest(void)
{
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case SET_IDLE:
            ...
            USB_CTRLDEVICE_HID_IDLE_RATE_CALLBACK(SetupPkt.W_Value.byte.LB, idle_rate);
            break;
        case GET_PROTOCOL:
            USBEP0SendRAMPtr(
