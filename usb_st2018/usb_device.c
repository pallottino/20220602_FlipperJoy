...
#if defined(USB_CTRLDEVICE_DISABLE_DTS_CHECKING)
...
// *****************************************************************************
// *****************************************************************************
USB_VOLATILE USB_CTRLDEVICE_STATE USBDeviceState;
...
#if !defined(USB_USER_CTRLDEVICE_DESCRIPTOR)
    //Device descriptor
    extern const USB_CTRLDEVICE_DESCRIPTOR device_dsc;
#else
    USB_USER_CTRLDEVICE_DESCRIPTOR_INCLUDE;
#endif
...
void USBStallEndpoint(uint8_t ep, uint8_t dir)
{
    ...
    {
        p = (BDT_ENTRY*)(&BDT[(uint8_t) EP(ep,dir,0)]);
        p->STAT.Val |= _BSTALL;
        p->STAT.Val |= _USIE;
        ...
        p = (BDT_ENTRY*)(&BDT[(uint8_t) EP(ep,dir,1)]);
        p->STAT.Val |= _BSTALL;
        p->STAT.Val |= _USIE;
        #endif
    }
...
static void USBCtrlEPServiceComplete(void)
{
    ...
    {
		if(SetupPkt.DataDir == USB_SETUP_CTRLDEVICE_TO_HOST_BITFIELD)
		{
		...
		else   // (SetupPkt.DataDir == USB_SETUP_DIRECTION_HOST_TO_CTRLDEVICE)
		{
			...
			//Although the data direction is HOST_TO_CTRLDEVICE, there is no data stage
			//(hence: outPipes[0].info.bits.busy == 0).  There is however still
			//an IN status stage.

			controlTransferState = CTRL_TRF_RX;     //Since this is a HOST_TO_CTRLDEVICE control transfer
...
static void USBStdGetDscHandler(void)
{
    if(SetupPkt.bmRequestType == 0x80)
    {
        ...
        {
            case USB_DESCRIPTOR_CTRLDEVICE:
                #if !defined(USB_USER_CTRLDEVICE_DESCRIPTOR)
                    inPipes[0].pSrc.bRom = (const uint8_t*)&device_dsc;
                #else
                    inPipes[0].pSrc.bRom = (const uint8_t*)USB_USER_CTRLDEVICE_DESCRIPTOR;
                #endif
...
static void USBStdGetStatusHandler(void)
{
    ...
    {
        case USB_SETUP_RECIPIENT_CTRLDEVICE_BITFIELD:
            inPipes[0].info.bits.busy = 1;
            /*
...
static void USBStdFeatureReqHandler(void)
{
    ...
    if((SetupPkt.bFeature == USB_FEATURE_CTRLDEVICE_REMOTE_WAKEUP)&&
       (SetupPkt.Recipient == USB_SETUP_RECIPIENT_CTRLDEVICE_BITFIELD))
    {
        inPipes[0].info.bits.busy = 1;
       ...
       (USBDeviceState == CONFIGURED_STATE))
    {
        ...
        //use for the next attempted transaction on that EP number.
            ...
            {
                p = (BDT_ENTRY*)((int) ((uintptr_t)p) & (~USB_NEXT_PING_PONG));
            }
...
/**************************************************************************
    ...
    Description:
        This function retrieves a 32-bit uint16_teger that normally increments by
