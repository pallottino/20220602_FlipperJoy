...
#define USB_UNKNOWN_CTRLDEVICE                      0x06    // Device with specified address is not attached.
...
#define USB_CTRLDEVICE_STATUS                       0x30                        // Offset for USBHostDeviceStatus() return codes
#define USB_CTRLDEVICE_ATTACHED                     (USB_CTRLDEVICE_STATUS | 0x30)  // Device is attached and running
#define USB_CTRLDEVICE_DETACHED                     (USB_CTRLDEVICE_STATUS | 0x01)  // No device is attached
#define USB_CTRLDEVICE_ENUMERATING                  (USB_CTRLDEVICE_STATUS | 0x02)  // Device is enumerating
#define USB_HOLDING_OUT_OF_MEMORY               (USB_CTRLDEVICE_STATUS | 0x03)  // Not enough heap space available
#define USB_HOLDING_UNSUPPORTED_CTRLDEVICE          (USB_CTRLDEVICE_STATUS | 0x04)  // Invalid configuration or unsupported class
#define USB_HOLDING_UNSUPPORTED_HUB             (USB_CTRLDEVICE_STATUS | 0x05)  // Hubs are not supported
#define USB_HOLDING_INVALID_CONFIGURATION       (USB_CTRLDEVICE_STATUS | 0x06)  // Invalid configuration requested
#define USB_HOLDING_PROCESSING_CAPACITY         (USB_CTRLDEVICE_STATUS | 0x07)  // Processing requirement excessive
#define USB_HOLDING_POWER_REQUIREMENT           (USB_CTRLDEVICE_STATUS | 0x08)  // Power requirement excessive
#define USB_HOLDING_CLIENT_INIT_ERROR           (USB_CTRLDEVICE_STATUS | 0x09)  // Client driver failed to initialize
#define USB_CTRLDEVICE_SUSPENDED                    (USB_CTRLDEVICE_STATUS | 0x0A)  // Device is suspended

#define USB_ERROR_CLASS_DEFINED                 0x50    // Offset for application defined errors

#define USB_SINGLE_CTRLDEVICE_ADDRESS               0x01    // Default USB device address (single device support)
...
The following flags are used in the flags parameter of the "USBCTRLDEVTransferData"
...
/*******************************************************************************
    ...
    Parameters:
        ...
        uint16_t size - Size of the event-specific data
...
typedef bool (*USB_EVENT_HANDLER) ( USB_EVENT event, void *data, uint16_t size );
